...
Strategy (backtested +10.65% improvement):
- When LONG opens → auto-open SHORT hedge (same size)
- When SHORT opens → auto-open LONG hedge (same size)
- At averaging step 2: Open gate, close 30% of hedge
- At averaging step 5: Close remaining 70% of hedge
- If hedge reverts to gate level: Close immediately (cut losses on hedge)
- If hedge hits 70% of peak profit: Surplus dump

Gate Logic:
- Gate opens when main position hits averaging step 2 or 5
- Tracks peak profit of hedge while gate is open
- Reversion = hedge profit drops back to gate level (with 5% buffer)
- Surplus dump = hedge profit drops to 70% of peak

Author: Claude + Grok Consultation
Version: 1.0.0
"""
...
class HedgeGateway:
    ...
    # Configuration (backtested optimal values)
    GATE_OPEN_STEPS = [2, 5]  # Averaging steps that open gates
    CLOSE_AT_STEP_2 = 0.30   # Close 30% of hedge at step 2
    CLOSE_AT_STEP_5 = 0.70   # Close 70% (remaining) at step 5
    GATE_BUFFER_PCT = 5.0    # 5% buffer for reversion detection
    SURPLUS_DUMP_PCT = 0.70  # Close at 70% of peak profit

    def __init__(self, exchange, enabled: bool = True):
        """
        ...
        print(f"🛡️ HedgeGateway initialized (enabled={enabled})")
    ...
    def _save_state(self):
        ...
        try:
            ...
            print(f"  ⚠️ Failed to save hedge state: {e}")
    ...
    def _load_state(self):
        ...
        try:
            ...
            print(f"  ⚠️ Failed to load hedge state: {e}")
    ...
    def calculate_hedge_profit(self, main_position_key: str,
        ...
        return size * pnl_pct * 10  # Assuming 10x leverage
